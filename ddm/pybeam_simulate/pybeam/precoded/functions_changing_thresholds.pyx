...
cdef double threshold(double phi[N_phi], double t):
    cdef double b = phi[2]
    cdef double m = phi[3]
    cdef double lamb = pow(10.0, phi[4])
    cdef double tau = pow(10.0, phi[5])
    cdef double lambtau = lamb*tau
    cdef double kappa = pow(10.0, phi[6])
    cdef double c = phi[7]
    cdef double thres = b - m*t - 0.5*b*(1.0 - c)*( 1.0 - exp( -pow( t/lambtau, kappa) ) )
    return thres
...
cdef double threshold_derivative(double phi[N_phi], double t):
    ...
    cdef double kappa = pow(10.0, phi[6])
    cdef double c = phi[7]
    cdef double thres_der = -m - 0.5*b*kappa*(1.0-c)/lambtau * pow(t/lambtau, kappa-1) * exp( -pow(t/lambtau, kappa) )
    return thres_der 
...
cdef double contamination_probability(double phi[N_phi], double t):
    cdef double gl = phi[9]
    cdef double gu = phi[10]
    cdef double pg = 0.0
    if (t >= gl) and (t <= gu):
        pg = 1.0/(gu - gl)
    return pg
