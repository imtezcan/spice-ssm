...
DEF delt = 6.04e-6 # numerical derivative step size
...
cpdef solveFP(phi, N_deps, dt_scale, fpt_bu, fpt_bl, output_loglike):
    ...
    dbudt_np1 = ( upper_decision_threshold(phic, t_in+delt) - upper_decision_threshold(phic, t_in-delt) ) / (2.0*delt)
    
    bl_np1 = lower_decision_threshold(phic, t_in);
    dbldt_np1 = ( lower_decision_threshold(phic, t_in+delt) - lower_decision_threshold(phic, t_in-delt) ) / (2.0*delt)
    ...
    for ii in range(N_dt):
        ...
        dbudt_np1 = ( upper_decision_threshold(phic, tt+delt) - upper_decision_threshold(phic, tt-delt) ) / (2.0*delt)
        ...
        dbldt_np1 = ( lower_decision_threshold(phic, tt+delt) - lower_decision_threshold(phic, tt-delt) ) / (2.0*delt)
...
cdef double approx_dt(double phi[N_phi], double dt_scale):
    ...
    # if no threshold crossings, set fpt to max
    if (fpt_avg == 0.0):
        fpt_avg = max_run_time*N_w
...
cpdef simulate_model_c(N_sims, dt, phi, seed):
    ...
    cdef double tt; # current time
    cdef double x_n; # location of walker at previous time step
    cdef double x_np1; # location of walker at previous time step
    # cdef double guess;
    # cdef double t_guess
    # cdef double side;
    ...
    cdef int N_sims_c = N_sims; # number of simulations
    cdef int seedc
    ...
    fpt = np.zeros(N_sims)
    x_array = np.zeros((N_sims, int(ceil(max_run_time/dt))))
    guess = 0.0
    side = 0.0
    u1 = 0.0
    u2 = 0.0
